  ...
  try {
    ...
    if (user.password) {
      return res.status(400).json({
        message: 'Password already set. Please log in using email and password.',
      });
    }
    ...
    return res.status(200).json({ message: 'Password set successfully. You can now log in using email and password.' });
  } catch (err) {
    console.error(err);
